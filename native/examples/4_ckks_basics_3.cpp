...
{   
    ...
    */
    ...
    for(j=0;j<6;j++){
        ...
        encrypted_fv.scale()=pow(2.0,50);
        encrypted_fw[j][0].scale()=pow(2.0,50);
    ...
    size_encrypted_prod = encrypted_f2.save(result_stream);
    print_line(__LINE__);
    cout << "Ciphertext (secret-key): wrote " << size_encrypted_prod << " bytes" << endl;
